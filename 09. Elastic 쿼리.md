...
# 국민청원 데이터 업로드
git clone https://github.com/lovit/petitions_archive
cd petitions_archive/
ls petitions_201* | xargs -i mv {} {}.json
 ls *.bat | xargs -i curl -u elastic:rlaxorud -k -H 'Content-Type: application/x-ndjson' -XPOST 'https://localhost:9200/bank/account/_bulk?pretty' --data-binary @{}

# 헤더추가
#python ./json2esbatch.py
import os

def json2esbatch(path, fname):
	with open(path) as f:
		data = f.read()

	data = data.replace('{"category"', '{"index":{"_index": "' + fname + '"}}\n{"category"')
	with open(path + ".bat", 'w') as f:
		f.write(data)

for root, dirs, files in os.walk("./"):
	for f in files:
		if f.startswith("petitions"):
			print(root + f)
			json2esbatch(root+f, f)
